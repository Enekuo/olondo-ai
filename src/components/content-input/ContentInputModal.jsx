...
const ContentInputModal = ({ isOpen, onOpenChange, onContentSubmit }) => {
  ...
  const processFile = (file) => {
    ...
    const isValidType = ALLOWED_FILE_TYPES.includes(file.type) ||
      (extension && ['pdf', 'txt', 'md', 'mp3'].includes(extension));
    if (!isValidType) {
      toast({
  ...
  const handleProcessPastedText = () => {
    ...
    toast({ title: t('contentInputModalTextProcessed', '¡Texto procesado con éxito!') });
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          <div
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {t('contentInputModalUploadSubtitle')} <span className="text-primary font-medium hover:underline">{t('contentInputModalUploadSubtitleLink')}</span>
            </p>
