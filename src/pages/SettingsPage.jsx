    ...
    <main className="w-full bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      <div className="mx-auto max-w-5xl px-6 md:px-10 lg:px-12 py-8 md:py-12">
        ...
        <form onSubmit={saveAll} className="space-y-6 md:space-y-8">
          ...
          {/* Audio */}
          <Section
            title={t("settings_audio_title") || "Audio por defecto"}
            description={t("settings_audio_desc") || "Configura la reproducción y la voz para tus escuchas."}
          >
            <Row>
              <Field label={t("settings_audio_voice") || "Voz"}>
                <select
                  className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-500"
                  value={audio.voice}
                  onChange={(e) => setAudio({ ...audio, voice: e.target.value })}
                >
                  <option value="olondo_standard_female">{t("settings_audio_voice_female") || "Olondo Femenina"}</option>
                  <option value="olondo_standard_male">{t("settings_audio_voice_male") || "Olondo Masculina"}</option>
                  <option value="olondo_podcast_pro">{t("settings_audio_voice_podcast") || "Podcast Pro"}</option>
                </select>
              </Field>

              <Field label={t("settings_audio_rate") || "Velocidad (x)"}
                     hint={t("settings_audio_rate_hint") || "0.5–1.5"}>
                <input
                  type="range"
                  min={0.5}
                  max={1.5}
            ...
            <Row>
              <Field label={t("settings_audio_pitch") || "Tono"}
                     hint={t("settings_audio_pitch_hint") || "-6 a +6"}>
                <input
                  type="range"
                  min={-6}
                  max={6}
              ...
              <div className="flex flex-col justify-end gap-3">
                <label className="inline-flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm">{t("settings_audio_autoplay_next") || "Reproducir el siguiente automáticamente"}</span>
                </label>
                ...
                <label className="inline-flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm">{t("settings_audio_normalize") || "Normalizar volumen automáticamente"}</span>
                </label>
              </div>
            </Row>
          </Section>
